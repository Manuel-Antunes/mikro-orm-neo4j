...
@Entity({
  ...
  comment: 'Production interface for movies and series.',
})
class Production {
  @PrimaryKey({ comment: 'The internal unique identifier for a production.' })
  id: string = crypto.randomUUID();

  @Property({ comment: 'The title of the production.' })
  title!: string;
}

@Entity({ labels: ['Movie', 'Production'], comment: 'A feature-length movie production.' })
class MovieProduction extends Production {
  @Property({ comment: 'The release year of the movie.' })
  released!: number;
}

@Entity({ labels: ['Series', 'Production'], comment: 'A television series production.' })
class SeriesProduction extends Production {
  @Property({ comment: 'The total number of episodes in the series.' })
  episodes!: number;
}
...
describe('Neo4j driver (MVP)', () => {
  ...
  describe('Real World Scenarios', () => {
    ...
    test('interface implementation with polymorphic queries', async () => {
      const movie = orm.em.create(MovieProduction, {
        title: 'The Matrix',
        released: 1999,
      });
      const series = orm.em.create(SeriesProduction, {
        title: 'Breaking Bad',
        episodes: 62,
      });
      ...
      const loadedMovies = await orm.em.find(MovieProduction, {});
      const loadedSeries = await orm.em.find(SeriesProduction, {});

      expect(loadedMovies).toHaveLength(1);
      expect(loadedSeries).toHaveLength(1);
      expect(loadedMovies[0].title).toBe('The Matrix');
      expect(loadedSeries[0].title).toBe('Breaking Bad');
